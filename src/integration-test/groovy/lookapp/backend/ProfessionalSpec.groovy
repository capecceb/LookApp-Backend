...
class ProfessionalSpec extends Specification {
    ...
    void "test update failed professional"(){
        ...
        body["name"] = "Diego"
        when: 'I try to update a professional'
        HttpResponse<Map> response = client.toBlocking().exchange(HttpRequest.PUT("/professonals/2",body), Map)
        then: 'The result is...'
    ...
    void "test add failed professional"(){
        ...
        then: 'The result is...'
        final HttpClientResponseException  exception = thrown()
        exception.message == "The propertie [name] of the class [lokapp-backend.Professional] can't be a null"
    }
